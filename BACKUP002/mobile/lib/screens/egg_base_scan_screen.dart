...
class _EggBaseScanScreenState extends State<EggBaseScanScreen> {
  ...
  Future<void> _doCapture({required bool isFace}) async {
    ...
    try {
      ...
      String outPath;
      if (isFace) {
        // Save full photo as face, with background masked to black
        outPath = '$_dir/face_$ts.jpg';
        final colorMat = cv.imdecode(bytes, 1);
        final masked = _maskEggBackground(colorMat);
        cv.imwrite(outPath, masked);
        masked.dispose();
        colorMat.dispose();
  ...
  /// Masque le fond de l'image : détecte le contour de l'œuf et peint
  /// tout le reste en noir opaque.
  cv.Mat _maskEggBackground(cv.Mat colorMat) {
    try {
      ...
      if (contours.length == 0) return colorMat;
      ...
      final masked = cv.bitwiseAND(colorMat, colorMat, mask: mask);
      mask.dispose();
          ...
          children: [
            GestureDetector(
              onScaleUpdate: (details) {
                final newZoom = (_currentZoom * details.scale).clamp(1.0, _maxZoom);
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Notes (optionnel)', hintText: 'Observations...'),
              maxLines: 3,
            ),
            const Spacer(),
