...
class _FirstScanScreenState extends State<FirstScanScreen>
  ...
  String? _analyzeDigit(Uint8List jpeg) {
    try {
      ...
      // Binarisation Otsu inverse : le chiffre gravé est plus foncé
      final (_, bin) = cv.threshold(gray, 0, 255,
          cv.THRESH_BINARY_INV + cv.THRESH_OTSU);
      gray.dispose();

      // Moments sur l'image binaire complète
      ...
      // Calcul manuel des 7 moments de Hu (non disponibles dans dartcv4)
      ...
      // Transformation log pour invariabilité d'échelle
      final huLog = hu.map((v) => -v.sign * math.log(v.abs() + 1e-10)).toList();

      // Références : valeurs approchées pour "2" et "5" dans la police utilisée.
      // À recalibrer avec les premières captures réelles.
